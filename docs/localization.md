...
Use `useLanguage().t()` for user-facing copy and maintain the natural Albanian version in `lib/localization.ts`. English phrases are stable dictionary keys. Translate presentation only: never change customer input, booking enum values, project slugs, URLs or brand names.
