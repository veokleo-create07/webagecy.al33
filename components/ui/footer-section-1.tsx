...
export default function Footer1() {
  ...
  const moveCta = (event: PointerEvent<HTMLAnchorElement>) => {
    ...
    ctaRef.current.style.setProperty("--cta-x", `${(event.clientX - rect.left - rect.width / 2) * .16}px`);
    ctaRef.current.style.setProperty("--cta-y", `${(event.clientY - rect.top - rect.height / 2) * .16}px`);
  };

  return <footer ref={footerRef} className={styles.footer} onPointerMove={onPointerMove} onPointerLeave={() => { footerRef.current?.style.setProperty("--footer-x", "0px"); footerRef.current?.style.setProperty("--footer-y", "0px"); footerRef.current?.style.setProperty("--footer-bend", "0deg"); }}>
    ...
    <motion.div className={styles.stage} initial="hidden" whileInView="visible" viewport={{ once: true, amount: .12 }}>
      <motion.div variants={reveal} className={styles.center}>
        <p className={styles.kicker}>KREU WEB</p>
        <h2>{t("We create digital experiences people remember.")}</h2>
        <a ref={ctaRef} href="#contact" className={styles.create} onPointerMove={moveCta} onPointerLeave={() => { ctaRef.current?.style.setProperty("--cta-x", "0px"); ctaRef.current?.style.setProperty("--cta-y", "0px"); }}><span>{t("Let’s create")}</span><ArrowIcon /></a>
      </motion.div>
      <motion.div variants={reveal} className={styles.pills}>
        ...
        <a className={styles.email} href="mailto:hello@kreuweb.com">hello@kreuweb.com</a>
      </motion.div>
      <motion.div variants={reveal} className={styles.bottom}>
        <p>© KREU WEB 2026 {t("All rights reserved.")}</p>
        <LanguageSwitcher />
        <a href="#top">{t("Back to top")} <ArrowIcon /></a>
      </motion.div>
    </motion.div>
  </footer>;
}
